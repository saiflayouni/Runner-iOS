...
class SignUpViewController: UIViewController {
   ...
   /*
    ...
    /*
     ...
     let verifyExistingEmailPath = "http://localhost:3000/user/register"
     
     let params = ["email": email]
     
     
     AF.request(verifyExistingEmailPath, method: .post, parameters: params, encoding: JSONEncoding.default, headers: nil).validate().responseJSON { response in
     
     switch response.result {
     case .success(let data):
     if let serverResponse = data as? [String:Any], serverResponse.keys.contains("message"), let isExisting = serverResponse["message"] as? Bool {
     if isExisting {
     DispatchQueue.main.async {
     // TODO: Display error message
     }
    ...
    @IBAction func createAccount(_ sender: UIButton) {
        ...
        if let email = emailTextField.text, !email.isEmpty {
            ...
            let registerPath = "http://localhost:3000/user/register"
            
            let params = ["email": email, "password": passwordTextField.text ?? ""]
            AF.request(registerPath, method: .post, parameters: params, encoding: JSONEncoding.default, headers: nil).validate().responseJSON { response in
                switch response.result {
                ...
                case .failure(let error):
                    ...
                    case .responseValidationFailed(let reason):
                        ...
                        case .unacceptableStatusCode(let code):
                            if code == 400 {
                                // Handle validation failure case (status code 400)
                                DispatchQueue.main.async {
                                    let alert = UIAlertController(title: "Error", message: "Invalid request body", preferredStyle: .alert)
                                    alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
                                    self.present(alert, animated: true, completion: nil)
                                }
                            }
                            else {
                                // Handle other unacceptable status codes (if any)
                                print("Response validation failed with code: \(code)")
                            }
                        default:
                            // Handle other response validation failure reasons (if any)
                            print("Response validation failed with reason: \(reason)")
                        }
                    default:
                        // Handle other Alamofire errors (if any)
